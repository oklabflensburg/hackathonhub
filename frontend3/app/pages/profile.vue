<template>
  <div class="py-8">
    <!-- Page Header using ProfileHeader Organism -->
    <ProfileHeader
      :title="$t('profile.title')"
      :subtitle="$t('profile.subtitle')"
      :show-edit-button="!!user && !isEditing"
      :is-editing="isEditing"
      :edit-button-text="$t('common.edit')"
      @edit="startEditing"
    />
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-4 sm:p-6 mb-6">
          ...
          <ClientOnly>
            <div v-if="user" class="space-y-6">
              <!-- Edit Mode using UserSettingsForm Organism -->
              <div v-if="isEditing">
                <UserSettingsForm
                  ...
                  :email="user?.email || ''"
                  :is-github-connected="!!user?.github_id"
                  :form-data="editForm"
                  :errors="{}"
                  :saving="saving"
                  :allow-avatar-upload="true"
                  :username-label="$t('profile.username')"
                  :name-label="$t('profile.name')"
                  :bio-label="$t('profile.bio')"
                  :location-label="$t('profile.location')"
                  :company-label="$t('profile.company')"
                  :github-connection-label="$t('profile.githubConnection')"
                  :connected-to-github-label="$t('profile.connectedToGitHub')"
                  :not-connected-to-github-label="$t('profile.notConnectedToGitHub')"
                  :connected-as-label="$t('profile.connectedAs', { username: user?.username || '' })"
                  :connect-github-to-link-identity-label="$t('profile.connectGitHubToLinkIdentity')"
                  :connect-github-button-label="$t('profile.connectGitHub')"
                  :connected-label="$t('profile.connected')"
                  :cancel-button-label="$t('common.cancel')"
                  :save-button-label="$t('common.save')"
                  @avatar-change="handleAvatarUpload"
