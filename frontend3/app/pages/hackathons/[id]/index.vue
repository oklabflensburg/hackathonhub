      ...
      <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div class="p-4 sm:p-5 lg:p-6">
          <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-bold text-gray-900 dark:text-white">{{ $t('hackathons.details.editHackathon') }}
            </h2>
          ...
          <form @submit.prevent="saveEdit" class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input v-model="editForm.end_date" type="datetime-local" required
                  class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-transparent" />
              </div>
            </div>
            ...
            <div>
              <div class="flex justify-between items-center mb-2">
                ...
                <button type="button" @click="editForm.prizes.push({ name: '', description: '', value: '' })"
              ...
              <div class="space-y-3">
                <div v-for="(prize, index) in editForm.prizes" :key="index"
                  ...
                  <div class="flex justify-between items-start mb-2">
                    <span class="font-medium text-gray-700 dark:text-gray-300">{{ $t('hackathons.details.prizeNumber', {
                      number: index + 1 }) }}</span>
                    <button type="button" @click="editForm.prizes.splice(index, 1)"
                ...
                <div v-if="editForm.prizes.length === 0"
            ...
            <div>
              <div class="flex justify-between items-center mb-2">
                ...
                <button type="button" @click="editForm.organizers.push({ name: '', role: '' })"
                  class="text-sm text-primary-600 dark:text-primary-400 hover:text-primary-700 dark:hover:text-primary-300">
              ...
              <div class="space-y-3">
                <div v-for="(organizer, index) in editForm.organizers" :key="index"
                  class="p-3 border border-gray-300 dark:border-gray-600 rounded-lg">
                  <div class="flex justify-between items-start mb-2">
                    <span class="font-medium text-gray-700 dark:text-gray-300">{{
                      $t('hackathons.details.organizerNumber', { number: index + 1 }) }}</span>
                    <button type="button" @click="editForm.organizers.splice(index, 1)"
                      class="text-red-600 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300">
                      {{ $t('hackathons.details.remove') }}
                    </button>
                  </div>
                ...
                <div v-if="editForm.organizers.length === 0"
                  class="text-center py-4 text-gray-500 dark:text-gray-400 border border-dashed border-gray-300 dark:border-gray-600 rounded-lg">
            ...
            <div>
              ...
              <input v-model="editForm.prize_pool" type="text"
                class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-transparent"
            ...
            <div class="flex justify-end space-x-3 pt-4">
              ...
              <button type="submit"
                ...
                :disabled="editLoading">
                <svg v-if="editLoading" class="w-5 h-5 mr-2 animate-spin" fill="none" viewBox="0 0 24 24">
                  <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" />
                  <path class="opacity-75" fill="currentColor"
                    d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
                </svg>
...
const saveEdit = async () => {
  ...
  try {
    ...
    const updateData: any = {
      ...
      prize_pool: editForm.value.prize_pool
    }

    // Convert arrays to JSON strings for API
    // Always send prizes and organizers, even if empty, to clear them
    updateData.prizes = JSON.stringify(editForm.value.prizes || [])

    updateData.organizers = JSON.stringify(editForm.value.organizers || [])
