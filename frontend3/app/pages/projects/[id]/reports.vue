<template>
  <div class="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8 space-y-6">
    <div v-if="!authStore.isAuthenticated" class="rounded-2xl border border-yellow-200 bg-yellow-50 p-6 text-yellow-900 dark:border-yellow-800 dark:bg-yellow-950/30 dark:text-yellow-100">
      You need to sign in to review project reports.
    </div>
    <div v-else-if="loading" class="py-20 text-center text-gray-500 dark:text-gray-400">Loading project reports...</div>
    ...
    <section v-else class="space-y-6">
      <header class="flex items-center justify-between gap-4">
        <div>
          ...
          <h1 class="text-3xl font-semibold text-gray-900 dark:text-white">Project Reports</h1>
        </div>
        <NuxtLink :to="`/projects/${projectId}`" class="btn btn-outline">Back to Project</NuxtLink>
      </header>
...
async function submitReview(payload: { status: 'pending' | 'reviewed' | 'resolved' | 'dismissed'; resolution_note?: string | null }) {
  ...
  try {
    ...
    uiStore.showSuccess('Project report updated', 'Moderation')
    closeReview()
  } catch (err: any) {
...
onMounted(() => { fetchReports('project', projectId.value).catch(() => undefined) })
</script>
