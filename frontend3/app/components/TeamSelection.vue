    ...
    <div v-else class="space-y-4">
      ...
      <div v-if="teams.length > 0">
        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
          {{ $t('teams.selectTeamForProject') }}
          <span class="text-gray-500 dark:text-gray-400 text-xs ml-1">({{ $t('common.optional') }})</span>
        </label>
        ...
        <div class="flex items-start">
          ...
          <div>
            <p class="text-blue-800 dark:text-blue-300 text-sm font-medium mb-1">
              {{ $t('teams.noTeamsForHackathon') }}
            </p>
            <p class="text-blue-700 dark:text-blue-400 text-sm">
              {{ $t('teams.createTeamToCollaborate') }}
            </p>
            <div class="mt-3">
              <NuxtLink
...
const teamOptions = computed(() => {
  const options = [
    { value: null, label: t('teams.noTeamIndividualProject') || 'No team - Individual project' }
  ]
  teams.value.forEach(team => {
    let label = team.name
    if (team.member_count !== undefined) {
      label += ` (${t('teams.memberCount', { count: team.member_count })})`
    }
    options.push({ value: team.id, label })
  })
  return options
})
