  ...
  <div class="flex flex-col md:flex-row md:items-start justify-between mb-8">
    <div>
      ...
      <p class="text-gray-600 dark:text-gray-400">
        ...
        <span v-else class="text-gray-500 dark:text-gray-400">{{ t('common.unknownHackathon') }}</span>
      </p>
    </div>

    <div class="mt-4 md:mt-0 flex space-x-3">
      <button v-if="isOwner" @click="$emit('edit')" class="btn btn-outline">{{ t('teams.editTeam') }}</button>
      <button v-if="isMember" @click="$emit('leave')" class="btn btn-outline text-red-600 dark:text-red-400 border-red-300 dark:border-red-700 hover:bg-red-50 dark:hover:bg-red-900/20">{{ t('teams.leaveTeam') }}</button>
      <button v-else-if="team.is_open && !isFull" @click="$emit('join')" class="btn btn-primary">{{ t('teams.joinTeam') }}</button>
    </div>
  </div>
</template>
...
defineProps<{ team: any; isOwner: boolean; isMember: boolean; isFull: boolean }>()
