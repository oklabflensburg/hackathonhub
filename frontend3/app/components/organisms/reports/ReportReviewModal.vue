<template>
  <Modal :model-value="visible" @update:model-value="value => { if (!value) $emit('close') }" @close="$emit('close')">
    <div class="space-y-5">
      <div>
        <h3 class="text-xl font-semibold text-gray-900 dark:text-white">Review Report</h3>
        <p class="mt-2 text-sm text-gray-600 dark:text-gray-300">{{ report?.resource?.name || `#${report?.resource_id}` }}</p>
      </div>
...
import type { ResourceReportSummary, ResourceReportStatus } from '~/composables/useReports'

const props = defineProps<{
  visible: boolean
  report: ResourceReportSummary | null
  loading?: boolean
}>()

const emit = defineEmits<{
  (e: 'close'): void
  (e: 'submit', payload: { status: ResourceReportStatus; resolution_note?: string | null }): void
}>()

const localStatus = ref<ResourceReportStatus>('pending')
const localNote = ref('')
