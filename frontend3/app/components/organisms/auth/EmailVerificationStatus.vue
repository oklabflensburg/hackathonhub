<template>
  <AuthCard
    :title="props.title"
    :subtitle="props.subtitle"
    :loading="props.loading"
    :disabled="props.disabled"
  >
    <template #default>
      <!-- Loading State -->
      <div v-if="props.loading" class="text-center">
        <div class="inline-block animate-spin rounded-full h-12 w-12 border-b-2 border-primary-600"></div>
        <p class="mt-4 text-gray-600 dark:text-gray-400">
          {{ props.loadingMessage }}
        </p>
      </div>

      <!-- Success Message -->
      <div v-else-if="props.verified" class="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-6">
        <div class="flex items-center">
          <svg class="w-8 h-8 text-green-600 dark:text-green-400 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
          </svg>
          <div>
            <p class="text-lg font-medium text-green-600 dark:text-green-400">
              {{ props.successTitle }}
            </p>
            <p class="text-sm text-green-600 dark:text-green-400 mt-1">
              {{ props.successMessage }}
            </p>
          </div>
        </div>
        <div class="mt-6">
          <NuxtLink
            :to="props.loginLink"
            class="w-full flex justify-center py-2 px-4 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white bg-primary-600 hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500"
          >
            {{ props.loginButtonText }}
          </NuxtLink>
        </div>
      </div>

      <!-- Error Message -->
      <div v-else-if="props.error" class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-6">
        <div class="flex items-center">
          <svg class="w-8 h-8 text-red-600 dark:text-red-400 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
          <div>
            <p class="text-lg font-medium text-red-600 dark:text-red-400">
              {{ props.errorTitle }}
            </p>
            <p class="text-sm text-red-600 dark:text-red-400 mt-1">
              {{ props.error }}
            </p>
          </div>
        </div>
        <div class="mt-6 space-y-3">
          <NuxtLink
            :to="props.loginLink"
            class="w-full flex justify-center py-2 px-4 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white bg-primary-600 hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500"
          >
            {{ props.loginButtonText }}
          </NuxtLink>
          <NuxtLink
            :to="props.registerLink"
            class="w-full flex justify-center py-2 px-4 border border-gray-300 dark:border-gray-700 rounded-lg shadow-sm text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500"
          >
            {{ props.registerButtonText }}
          </NuxtLink>
        </div>
      </div>

      <!-- No Token Message -->
      <div v-else class="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-6">
        <div class="flex items-center">
          <svg class="w-8 h-8 text-yellow-600 dark:text-yellow-400 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.998-.833-2.768 0L4.342 16.5c-.77.833.192 2.5 1.732 2.5z" />
          </svg>
          <div>
            <p class="text-lg font-medium text-yellow-600 dark:text-yellow-400">
              {{ props.noTokenTitle }}
            </p>
            <p class="text-sm text-yellow-600 dark:text-yellow-400 mt-1">
              {{ props.noTokenMessage }}
            </p>
          </div>
        </div>
        <div class="mt-6">
          <NuxtLink
            :to="props.loginLink"
            class="w-full flex justify-center py-2 px-4 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white bg-primary-600 hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500"
          >
            {{ props.loginButtonText }}
          </NuxtLink>
        </div>
      </div>
    </template>
  </AuthCard>
</template>

<script setup lang="ts">
import AuthCard from '~/components/atoms/AuthCard.vue'

interface Props {
  // UI state
  loading?: boolean
  disabled?: boolean
  verified?: boolean
  error?: string
  
  // Content
  title?: string
  subtitle?: string
  loadingMessage?: string
  successTitle?: string
  successMessage?: string
  errorTitle?: string
  noTokenTitle?: string
  noTokenMessage?: string
  loginButtonText?: string
  registerButtonText?: string
  loginLink?: string
  registerLink?: string
}

const props = withDefaults(defineProps<Props>(), {
  loading: false,
  disabled: false,
  verified: false,
  error: undefined,
  title: 'Email Verification',
  subtitle: 'Verifying your email address...',
  loadingMessage: 'Verifying your email address...',
  successTitle: 'Email Verified Successfully!',
  successMessage: 'Your email address has been verified. You can now log in to your account.',
  errorTitle: 'Verification Failed',
  noTokenTitle: 'No Verification Token',
  noTokenMessage: 'No verification token was provided. Please check your email for the verification link.',
  loginButtonText: 'Go to Login',
  registerButtonText: 'Register Again',
  loginLink: '/login',
  registerLink: '/register'
})
</script>