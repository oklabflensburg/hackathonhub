...
class PushNotificationService:
    ...
    def send_push_notification(
        ...
        try:
            ...
            if not self._should_send_push_notification(db, user_id, notification_type):
                logger.debug(
                    f"Push notifications disabled for user {user_id}, type {notification_type}")
                return False

            # Get user's push subscriptions
            subscriptions = crud.get_user_push_subscriptions(db, user_id)
            ...
            for subscription in subscriptions:
                try:
                    ...
                    logger.error(
                        f"Failed to send to subscription {subscription.endpoint}: {e}")
                    failed_subscriptions.append(subscription.endpoint)
            ...
            if success_count > 0:
                self._log_notification(
                    db, user_id, notification_type, title, body, data, "push")
                logger.info(
                    f"Sent push notification to {success_count} device(s) for user {user_id}")
                return True
            else:
                logger.warning(
                    f"Failed to send push notification to any device for user {user_id}")
                return False
    ...
    def _should_send_push_notification(self, db: Session, user_id: int, notification_type: str) -> bool:
        """Check if push notification should be sent for this user and notification type."""
        # Use the notification preference service to check
        try:
            ...
            # Fallback to simple check if service not available
            logger.warning(
                "Notification preference service not available, using default behavior")
            return True

    def _create_notification_payload(self, title: str, body: str, data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    def _send_to_subscription(self, subscription: models.PushSubscription, payload: Dict[str, Any], ttl: int) -> bool:
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                logger.warning(
                    f"Failed to send push notification to {endpoint}: {response.status_code}")
                return False
    ...
    def _cleanup_failed_subscriptions(self, db: Session, user_id: int, failed_endpoints: List[str]):
        """Remove failed push subscriptions from database."""
        for endpoint in failed_endpoints:
            try:
                crud.delete_push_subscription(db, user_id, endpoint)
                logger.info(f"Removed failed push subscription: {endpoint}")
            except Exception as e:
                logger.error(
    ...
    def send_bulk_push_notifications(
        ...
        for user_id in user_ids:
            try:
                if self.send_push_notification(db, user_id, notification_type, title, body, data):
                    results["success"] += 1
                else:
                    results["failed"] += 1
            except Exception as e:
    ...
    def generate_vapid_keys(self) -> Dict[str, str]:
        ...
        try:
            ...
            private_b64 = base64.urlsafe_b64encode(
                private_bytes).decode('utf-8').rstrip('=')
